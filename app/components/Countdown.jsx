...
// Base background
context.fillStyle = 'rgba(255, 105, 180, 0.25)'; // pink
context.fillRect(0, 0, cw, ch);

// Conic gradient overlay
const gradient = context.createConicGradient(0, centerx, centery);

// Pink → Hot-Pink → Red → Hot-Pink → Pink (smooth loop)
gradient.addColorStop(0.0,  'rgba(255, 105, 180, 0.15)'); // pink
gradient.addColorStop(0.25, 'rgba(255, 20, 147, 0.15)');  // hot-pink
gradient.addColorStop(0.60,  'rgba(220, 20, 60, 0.15)');   // red
gradient.addColorStop(0.75, 'rgba(255, 20, 147, 0.15)');  // hot-pink
gradient.addColorStop(1.0,  'rgba(255, 105, 180, 0.15)'); // pink
...
function triggerExpanse() {
  ...
  // Fade out the center hover immediately
  centerHover.style.transition = 'opacity 0.5s ease';
  centerHover.style.opacity = 0;
  centerHover.style.pointerEvents = 'none';

  // Prepare container transition BEFORE changing transform/opacity
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      returning = false;
      window.location.href = 'https://www.youtube.com/watch?v=i2Q5WzL0z1Y';
    }, 2000); 
  }, 1000); 
}
    ...
    <>
      ...
      <div
        style={{
          ...
          zIndex: 0,
          background:
            'conic-gradient(from 0deg, rgba(0,0,0,0) 0%, #ffffff 50%, rgba(255,255,255,0) 100%)',
          filter: 'blur(120px)',
